...
def bezier(t, a, b, c, d):  # 贝塞尔函数（估测长度用）
    return (-a + 3 * b - 3 * c +
            d) * t * t * t + 3 * (a - 2 * b + c) * t * t + 3 * (-a + b) * t + a
...
def prSolve(m, cs, ce, n):  # 主要计算函数1-解析解
    ...
    if n == 0:
        return 1j * oneOver2PI / m * (cmath.exp(-m * 1j * ce) -
                                      cmath.exp(-m * 1j * cs))
    elif n > 0:
        return 1j * oneOver2PI / m * cmath.exp(-m * 1j * ce) - n * 1j / (
            (ce - cs) * m) * prSolve(m, cs, ce, n - 1)
    else:
        return 0


def numSolve(m, cs, ce, pts):  # 主要计算函数2-贝塞尔曲线方程代入
    return (-pts[0] + 3 * pts[1] - 3 * pts[2] + pts[3]) * prSolve(
        m, cs, ce, 3) + 3 * (pts[0] - 2 * pts[1] + pts[2]) * prSolve(
            m, cs, ce, 2) + 3 * (-pts[0] + pts[1]) * prSolve(
                m, cs, ce, 1) + pts[0] * prSolve(m, cs, ce, 0)
...
if __name__ == '__main__':
    ...
    for curve in points:  # Calculate weight
        ...
        for i in range(1, wst):
            sum += abs(
                bezier(linear(i, 0, wst, 0, 1), curve[0], curve[1], curve[2],
                       curve[3]) -
